...
#### Продуктовый CTA внутри статьи

Для заметной ссылки на приложение в середине текста использовать `InlineConsultation` с `variant="product"`: существующий светло-голубой фон заметки, синяя боковая линия, краткая подводка по теме раздела и одна синяя кнопка. Отступы наследуются от общего ритма статьи. На мобильном кнопка занимает ширину блока. В статье достаточно одной такой вставки, до FAQ и источников; нижний продуктовый CTA сохраняется. Текст обозначает назначение приложения и условия перехода к нему без обещаний неподтверждённых возможностей.

#### Компактный ритм статьи

Источник отступов — `--article-space-*` в `article-layout.module.scss` и общие редакционные компоненты. На desktop и mobile сохраняется одна иерархия:

| Переход | Desktop | Mobile ≤ 768 px |
| --- | --- | --- |
| Между абзацами | 16 px | 16 px |
| После H2 / после H3 | 20 / 12 px | 20 / 12 px |
| Перед H3 | 32 px | 24 px |
| Вокруг таблицы, изображения, заметки, чек-листа и встроенной консультации | 24 px | 24 px |
| Между основными разделами, включая FAQ | 32 px + линия 1 px + 32 px | 24 px + линия 1 px + 24 px |
| От вступления или заметки до первого раздела | 32 px | 24 px |
| Внешние отступы тела статьи и нижних блоков | 48 px | 32 px |

- Соседние обычные блочные отступы схлопываются; не суммировать их через дополнительные обёртки или пустые элементы.
- Последний элемент основного раздела не добавляет нижний margin: переход к следующему разделу задаёт сам раздел.
- Разделители и межсекционные отступы относятся только к верхнему уровню статьи; вложенный чек-лист сохраняет свои отступы.
- Подпись изображения отделена на 12 px; строки источников — на 8 px. Компактность не уменьшает кликабельные области FAQ и кнопок.
- Проверять вступление без заметки, заголовки после таблиц и консультаций, вложенный чек-лист, FAQ и переход к источникам на 390, 768 и 1440 px. Не использовать индивидуальные поправки в страницах.
