...
Путь-пример нужно проверить в FTP-клиенте: FTP-корень может отличаться от SSH-корня. Не копируйте каталог sk-rosa.ru. Используется явный FTPS на стандартном порту 21 с проверкой сертификата; сервер должен поддерживать FTPS.
